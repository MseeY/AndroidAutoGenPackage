...
public class TestMain {
	...
	private static File decompileFile;//反编译后的路径
	...
	private static void readConfigChanel() {
		...
		try {
			...
			Element rootElement = chanelDocument.getDocumentElement();
	...
	private static void configInitInfo(Element rootElement,Element channelElement) {
		currentChannelName = channelElement.getAttribute("name");
		currentChanelPath = new File(implSdkPath, currentChannelName);
	...
	private static void decompileApk() {
		...
		decompileFile = new File(tempPath, "decompile");// 反编译后的路径
	 ...
	 */
	private static void configManifest() {
		startTime = System.currentTimeMillis();
		System.out.println("配置AndroidManifest.xml" + "--执行开始.."+System.currentTimeMillis());
		File  tempManifestPath = new File(decompileFile,"AndroidManifest.xml");
		try {
	 ...
	 */
	private static void decompileDex2Smali(){
		 startTime = System.currentTimeMillis();
	 ...
	 * @param name
	 */
 	private static void decompileR2Smali(){
		 startTime = System.currentTimeMillis();
		...
		try {
			...
			File newFile = new File(tempPath,"public.xml");
			if(oldFile.exists()){
